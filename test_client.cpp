...
void run_upsert_test_benchmark(int server_sock, _key_t* upsert_keys, size_t upsert_times, size_t batch_size, Client_message::Type type) {
    TaskScheduler scheduler;
    ...
    std::thread prepThread([&scheduler, upsert_times, upsert_keys, batch_size, type]() {
        std::normal_distribution<_key_t> key_dist(0, 1e9);
        std::mt19937 gen(time(NULL)); 
        // std::uniform_int_distribution<size_t> dist(0, upsert_times - 1);
        std::mt19937 payload_gen(time(NULL));
...
// void run_BPlus_tree(int sock, _key_t* keys, _payload_t* payloads, size_t number, size_t test_size, size_t batch_size) {
//     TaskScheduler scheduler;
    
//     auto start = std::chrono::high_resolution_clock::now();

//     // 数据准备线程
//     std::thread prepThread([&scheduler, keys, payloads, number, test_size, batch_size]() {
//         std::mt19937 gen(time(NULL));
//         std::uniform_int_distribution<size_t> dist(0, number - 1);
...
//             Client_message msg(Client_message::BPlus, batch_keys, real_batch_size);
...
// void run_ALEX(int sock, _key_t* keys, _payload_t* payloads, size_t number, size_t test_size, size_t batch_size) {
//     TaskScheduler scheduler;
...
//             Client_message msg(Client_message::ALEX_lookup, batch_keys, real_batch_size);
//             std::string message = msg.serialize();
...
int main(int argc, char* argv[]) {
    ...
    if (mode == 0) {
        ...
        std::cout << "run B Tree search" << std::endl;
        run_search_test_benchmark(server_sock, keys, payloads, number, test_size, batch_size, Client_message::BTree_lookup);
    }

    else if (mode == 5) {
        std::cout << "run B Tree upsert" << std::endl;
        run_upsert_test_benchmark(server_sock, keys, number, batch_size, Client_message::BTree_upsert);
    }

    else if (mode == 6) {
        std::cout << "run B Plus Tree search" << std::endl;
        run_search_test_benchmark(server_sock, keys, payloads, number, test_size, batch_size, Client_message::BPtree_lookup);
    }

    else if (mode == 7) {
        std::cout << "run B Plus Tree upsert" << std::endl;
        run_upsert_test_benchmark(server_sock, keys, number, batch_size, Client_message::BPtree_upsert);
    }

    else if (mode == 8) {
        std::cout << "run ALEX search" << std::endl;
        run_search_test_benchmark(server_sock, keys, payloads, number, test_size, batch_size, Client_message::ALEX_lookup);
    }

    else if (mode == 9) {
        std::cout << "run ALEX insert" << std::endl;
        run_upsert_test_benchmark(server_sock, keys, number, batch_size, Client_message::ALEX_upsert);
    }

    else if (mode == 10) {
        std::cout << "run all lookup tests" << std::endl << std::endl;
        std::cout << "run lookup with cache" << std::endl;
        run_search_test_client(server_sock, testindex, keys, payloads, number, test_size, batch_size, true);

        std::cout << "run lookup without cache" << std::endl;
        run_search_test_client(server_sock, testindex, keys, payloads, number, test_size, batch_size, false);
        ...
        std::cout << "run all upsert tests" << std::endl << std::endl;
        ...
        std::cout << "run B Tree upsert" << std::endl;
        run_search_test_benchmark(server_sock, keys, payloads, number, test_size, batch_size, Client_message::BTree_upsert);
