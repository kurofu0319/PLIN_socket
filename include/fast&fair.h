    ...
    public:
        ...
        bool remove_rebalancing(btree* bt, _key_t key, bool only_rebalance = false) {
            ...
            if(is_leftmost_node) {
                ...
                page * sibling = (page *) hdr.sibling_ptr;
            ...
            while(left_sibling->hdr.sibling_ptr != (page *)this) {
                left_sibling = (page *)left_sibling->hdr.sibling_ptr;
            }
            ...
            if(total_num_entries > cardinality - 1) { // Redistribution
                ...
                if(num_entries < left_num_entries) { // left -> right
                    if(hdr.leftmost_ptr == NULL){
                        ...
                        (char *)&(left_sibling->records[m].val), sizeof(char *);

                        left_sibling->hdr.last_index = m - 1;
                        (char *)&(left_sibling->hdr.last_index), sizeof(int16_t);
                        ...
                        (char *)&(hdr.leftmost_ptr), sizeof(page *);
                    ...
                    hdr.is_deleted = 1;
                    (char *)&(hdr.is_deleted), sizeof(uint8_t);
                ...
                (char *)&(left_sibling->hdr.sibling_ptr), sizeof(page *);
            }
        ...
        inline void 
            insert_key(_key_t key, char * ptr, int *num_entries, bool flush = false,
                    bool update_last_index = false) {
...
bool flush, page *invalid_sibling = NULL) {
                ...
                if(num_entries < cardinality - 1) {
                    insert_key(key, hdr.leftmost_ptr == NULL ? (char *)right: (char *)right, &num_entries, flush);
                    return this;
                }
                else {// FAIR
                    // overflow
                    ...
                    // insert the key
                    if(key < split_key) {
                        insert_key(key, hdr.leftmost_ptr == NULL ? (char *)right: (char *)right, &num_entries);
                        ret = this;
                    }
                    else {
                        sibling->insert_key(key, hdr.leftmost_ptr == NULL ? (char *)right: (char *)right, &sibling_cnt);
                        ret = sibling;
                    }
...
bool btree::find(_key_t key, _payload_t & val){
    ...
    {
        std::cout << "finding KEY 35497497.707480 in overflowtree" << std::endl;
...
void btree::insert(_key_t key, _payload_t right){ // need to be std::string
    ...
    if(!p->store(this, NULL, key, (char *)right, false)) { // store 
        insert(key, right);
    }
}
...
void btree::btree_insert_internal(char *left, _key_t key, char *right, uint32_t level) {
    ...
    if(!p->store(this, NULL, key, right, false)) {
...
uint32_t btree::upsert(_key_t key, _payload_t value, uint32_t ds){
    ...
    if(!t) {
        ...
        if(!first_p->store(this, NULL, key, (char *)value, false)) {
            insert(key, value);
        }
        return 4;
    }
...
btree::btree(void** addr, bool init)
    ...
    if (init) {
        ...
        *addr = root;
        ...
        //std::cout << "btree false init" << std::endl;
        root = (page*) *addr;
    }
}
